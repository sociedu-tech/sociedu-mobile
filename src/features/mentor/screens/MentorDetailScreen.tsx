...
export default function MentorDetailScreen() {
  ...
  useEffect(() => {
    const loadMentor = async () => {
      ...
      try {
        ...
        setError(err.message || 'Khong the tai chi tiet mentor.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCheckout = async () => {
    if (!selectedVer) {
      Alert.alert('Loi', 'Vui long chon mot goi dich vu.');
      return;
    }
    ...
    try {
      ...
      if (!order.paymentUrl) {
        throw new Error('Khong lay duoc URL thanh toan.');
      }
      ...
      if (finalOrder.status === 'paid') {
        Alert.alert('Thanh cong', 'Thanh toan thanh cong. Lich hen da duoc tao.', [
          { text: 'Xem lich hen', onPress: () => router.replace('/(tabs)/bookings') },
        ]);
      } else {
        Alert.alert('Cho xac nhan', 'Vui long kiem tra tab Lich hen sau it phut.', [
          { text: 'Dong', onPress: () => router.replace('/(tabs)/bookings') },
        ]);
      }
    } catch (err: any) {
      Alert.alert('Loi thanh toan', err.message || 'Da co loi xay ra.');
    } finally {
  ...
  if (loading) {
    return <LoadingState message="Dang tai du lieu Mentor..." />;
  }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <View style={styles.headerBar}>
        ...
        <Typography variant="bodyMedium" style={{ fontWeight: '700' }}>Chi tiet</Typography>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Typography variant="body" style={styles.bio}>
          {mentor.bio || 'Chua co thong tin gioi thieu.'}
        </Typography>

        <View style={styles.divider} />

        <Typography variant="h3" style={{ marginBottom: 12 }}>Cac goi dich vu</Typography>

        {info?.packages?.length === 0 ? (
          <Typography variant="bodyMedium" color="secondary">Mentor chua cung cap goi dich vu nao.</Typography>
        ) : (
      ...
      {selectedVer && (
        <View style={styles.footerWrap}>
          <View style={{ flex: 1 }}>
            <Typography variant="caption" color="secondary">Tong thanh toan</Typography>
